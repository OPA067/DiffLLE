...
if __name__ == "__main__":
    ...
    parser.add_argument('-c', '--config', type=str, default='config/light_LOL_v1.json', help='JSON file for configuration')
    ...
    logger.info('Train/Test Datasets len is {}/{}'.format(len(train_set), len(val_set)))
    # model
            ...
            for _, train_data in enumerate(train_loader):
                ...
                if current_step % opt['train']['val_freq'] == 0:
                    ...
                    for _,  val_data in enumerate(val_loader):
                        ...
                        tb_logger.add_image('Iter_{}'.format(current_step), np.transpose(np.concatenate((sr_img, hr_img), axis=1), [2, 0, 1]), idx)
                        ...
                        logger.info('# Validation id {}, # PSNR: {:.4e}, #SSIM:{:.4e}'.format(idx, Metrics.calculate_psnr(sr_img, hr_img), Metrics.calculate_ssim(sr_img, hr_img)))
                    ...
                    # log
                    logger.info('# Total Validation # PSNR: {:.4e}'.format(avg_psnr))
                    logger.info('# Total Validation # SSIM: {:.4e}'.format(avg_ssim))
